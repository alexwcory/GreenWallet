...
interface MRegister {
    function getAllMintedSBTs(
        address _address
    ) external view returns (string[] memory _tiersOwned);
...
contract GreenWallet {
    ...
    function addIntoContract(
        address _walletAddress,
        string[] memory _chains,
        int[] memory numTransactions
    ) public returns (int score) {
        require(
            !addressExists(_walletAddress),
            "This wallet already exists in this contract"
        );
        require(
            _chains.length == numTransactions.length,
            "Categories and values arrays must be of equal length"
        );
        ...
        int score_val = MMan.getScore(_walletAddress);
    ...
    function getScore(
        address _walletAddress
    ) public view returns (int walletScore) {
        require(
            addressExists(_walletAddress),
            "This wallet does not exist in this contract"
        );
    ...
    function getNumTransactions(
        address _walletAddress
    ) public view returns (int[] memory _transactionList) {
    ...
    function walletOwns(
        ...
        string[] memory mintedSBTs = MReg.getAllMintedSBTs(_walletAddress);
        string memory concatenated = "";
        for (uint256 i = 0; i < mintedSBTs.length; i++) {
            concatenated = string(
                abi.encodePacked(concatenated, mintedSBTs[i], ";")
            );
        }
        return concatenated;
    }
